...
export function registerAsmLanguage(monaco: Monaco) {
  ...
  monaco.languages.setMonarchTokensProvider('asm', {
    ...
    tokenizer: {
      root: [
        ...
        [/(\^program)(\s+)([a-zA-Z_][\w$]*)/, ['preprocessor', 'white', 'preprocessor.param']],
        [/(\^program)(\s+)([a-zA-Z_][\w$]*)(\s+)(.*)$/, ['preprocessor', 'white', 'preprocessor.param', 'white', 'preprocessor.value']],
        [/(\^declare)(\s+)([a-zA-Z_][\w$]*)/, ['preprocessor', 'white', 'variable.declaration']],
        [/(\^const)(\s+)([a-zA-Z_][\w$]*)(\s+)(.*)$/, ['preprocessor', 'white', 'variable.declaration', 'white', 'constant']],
        ...
        [new RegExp(`\\b(${AsmKeywords.programFlow.join('|')})\\b`), 'keyword.control'],

        // Opcodes - Stack Operations
        [new RegExp(`\\b(${AsmKeywords.stackOperations.join('|')})\\b`), 'keyword.stack'],

        // Opcodes - Arithmetic Operations
        [new RegExp(`\\b(${AsmKeywords.arithmeticOperations.join('|')})\\b`), 'keyword.operator'],

        // Opcodes - API Calls
        [new RegExp(`\\b(${AsmKeywords.apiCalls.join('|')})\\b`), 'keyword.api'],

        // Opcodes - Memory Operations
        [new RegExp(`\\b(${AsmKeywords.memoryOperations.join('|')})\\b`), 'keyword.memory'],
  ...
  monaco.languages.registerCompletionItemProvider('asm', {
    provideCompletionItems: (model: any, position: any) => {
      ...
      // Check if we're typing after "FUN" or "API" to provide function suggestions
      ...
      const completions = [
        // Program Flow Operations with BEQ added
        ...createCompletionItems(AsmKeywords.programFlow, monaco.languages.CompletionItemKind.Keyword, range, 'Program Flow Operation'),

        // Add preprocessor directives to completions
        {
          label: '^program',
          ...
          documentation: 'Define program name'
        },
        {
          ...
          documentation: 'Define program description'
        },
        {
          label: '^program activationAmount',
          kind: monaco.languages.CompletionItemKind.Snippet,
          insertText: '^program activationAmount ${1:amount}',
          insertTextRules: monaco.languages.CompletionItemInsertTextRule.InsertAsSnippet,
          range,
          detail: 'Program Activation Amount',
          documentation: 'Set program activation amount'
        },
        {
          label: '^declare',
          kind: monaco.languages.CompletionItemKind.Snippet,
        ...
        // Stack Operations
        ...createCompletionItems(AsmKeywords.stackOperations, monaco.languages.CompletionItemKind.Keyword, range, 'Stack Operation'),

        // Arithmetic Operations
        ...createCompletionItems(AsmKeywords.arithmeticOperations, monaco.languages.CompletionItemKind.Operator, range, 'Arithmetic Operation'),

        // API Calls
        ...createCompletionItems(AsmKeywords.apiCalls, monaco.languages.CompletionItemKind.Function, range, 'API Call'),

        // Memory Operations
        ...createCompletionItems(AsmKeywords.memoryOperations, monaco.languages.CompletionItemKind.Method, range, 'Memory Operation'),

        // Data Types
        ...createCompletionItems(AsmKeywords.dataTypes, monaco.languages.CompletionItemKind.TypeParameter, range, 'Data Type'),
  ...
  monaco.editor.defineTheme('asm-light', {
    ...
    rules: [
      ...
      { token: 'preprocessor.value', foreground: '#666666', fontStyle: 'italic' },
...
// Define a dark theme for the language
  monaco.editor.defineTheme('asm-dark', {
    ...
    rules: [
      ...
      { token: 'preprocessor.value', foreground: '#AAAAAA', fontStyle: 'italic' },
...
function createCompletionItems(items: string[], kind: any, range: any, detail: string) {
